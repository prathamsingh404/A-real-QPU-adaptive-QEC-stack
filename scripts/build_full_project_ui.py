"""
Builds the complete AdaptiveQEC interface inside the exact template from generated-page.html.
Preserves:
  - 3D WebGL background hero substrate (heroGL)
  - 3D Three.js cryostat assembly (asmGL)
  - 3D Three.js anneal fault-tolerant stage (annGL)
  - Interactive loupe magnification canvas (loupeStage)
  - Interactive caustics probability wave canvas (caustics)
  - Draggable 3D tilt cards with shards (notesRail)
  - WebGL liquid-metal CTA buttons with fluid shader
  - Boot loading screen (boot)
  - HUD scroll progress ticker (hud)
  - 3D perspective footer brand mark (fMark)

Replaces and enhances all content to implement:
  - IBM Eagle r3 27-Qubit Heavy-Hex Architecture & Live Telemetry
  - Interactive 27-Transmon Coupling SVG Lattice
  - Live Stim Syndrome Extraction Loupe
  - Heterogeneous Decoder Benchmarks (MWPM, Union-Find, ML, Adaptive Router)
  - Modern Chart.js Multi-Axis Accuracy/Latency Graph
  - In-Situ CUSUM & EWMA Drift Detection Chart
  - Dilution Refrigerator 3D Assembly Callouts
  - Selective Recalibration Engine (58.4% shot reduction)
  - Sim-to-Hardware Reality Gap & Failure Mode Clustering
  - Live QEC Experiment Workbench with active POST /api/qec/run execution
  - OpenQASM 3.0 & Stim Clifford Export
  - Significantly larger, high-contrast, luminous typography
"""
...
# =========================================================================
# 1. ENHANCE TYPOGRAPHY: MAKE ALL VISIBLE TEXT MUCH BIGGER & HIGH CONTRAST
# =========================================================================

# Contrast & tokens
html = html.replace('--dim:rgba(242,244,247,.55);', '--dim:rgba(242,244,247,.82);')
html = html.replace('--faint:rgba(242,244,247,.34);', '--faint:rgba(242,244,247,.62);')

# Big Typography
html = html.replace(
    'h1{font-size:clamp(50px,6.4vw,96px);font-weight:500;letter-spacing:-.034em;line-height:1.0;color:#fff}',
    'h1{font-size:clamp(56px,7.2vw,104px);font-weight:600;letter-spacing:-.034em;line-height:1.02;color:#ffffff}'
)
html = html.replace(
    'h2{font-size:clamp(32px,4.2vw,60px);font-weight:500;letter-spacing:-.028em;line-height:1.06}',
    'h2{font-size:clamp(38px,4.8vw,68px);font-weight:600;letter-spacing:-.028em;line-height:1.08;color:#ffffff}'
)
html = html.replace(
    'p{font-size:clamp(15px,1.12vw,17px);line-height:1.66;color:var(--dim)}',
    'p{font-size:clamp(17px,1.25vw,20px);line-height:1.72;color:rgba(242,244,247,.84)}'
)
html = html.replace(
    '.mono{font-family:var(--mono);font-size:10px;letter-spacing:.26em;text-transform:uppercase}',
    '.mono{font-family:var(--mono);font-size:11.5px;letter-spacing:.26em;text-transform:uppercase}'
)
html = html.replace(
    '.data{font-size:clamp(28px,2.7vw,44px);font-weight:400;letter-spacing:-.022em;\nfont-variant-numeric:tabular-nums}',
    '.data{font-size:clamp(34px,3.2vw,52px);font-weight:500;letter-spacing:-.022em;color:#fff;font-variant-numeric:tabular-nums}'
)
html = html.replace(
    '.lead{max-width:46ch;margin-top:26px}',
    '.lead{max-width:54ch;margin-top:26px;font-size:clamp(18px,1.35vw,22px);line-height:1.66;color:rgba(242,244,247,.88)}'
)
html = html.replace(
    '.row .nm{font-size:16px;color:var(--dim);transition:color .35s}',
    '.row .nm{font-size:18px;font-weight:500;color:rgba(242,244,247,.88);transition:color .35s}'
)
html = html.replace(
    '.stats b{display:block;font-size:clamp(26px,2.4vw,38px);font-weight:500;letter-spacing:-.026em;\nfont-variant-numeric:tabular-nums}',
    '.stats b{display:block;font-size:clamp(32px,3.0vw,46px);font-weight:600;letter-spacing:-.026em;color:#fff;font-variant-numeric:tabular-nums}'
)
html = html.replace(
    '.stats span{color:var(--faint);font-family:var(--mono);font-size:9.5px;letter-spacing:.24em;',
    '.stats span{color:var(--faint);font-family:var(--mono);font-size:11.5px;letter-spacing:.24em;'
)
html = html.replace(
    '.line .w{font-size:clamp(22px,2.1vw,32px);margin-top:16px;letter-spacing:-.02em;\nfont-variant-numeric:tabular-nums}',
    '.line .w{font-size:clamp(26px,2.6vw,38px);margin-top:16px;letter-spacing:-.02em;color:#fff;font-variant-numeric:tabular-nums}'
)
html = html.replace(
    ...
    '.cap-i{display:flex;align-items:baseline;gap:14px;padding:15px 0;border-bottom:1px solid var(--hair);\ncolor:var(--dim);font-size:17.5px}'
)

# Expand hero content width and story height for bigger readable text
html = html.replace(
    ...
    '.hero-content{position:absolute;z-index:30;left:var(--pad);top:50%;\ntransform:translateY(-50%) translateY(var(--exit,0px));width:min(52vw,700px)}'
)
html = html.replace(
    '.hero-story{position:relative;margin-top:26px;max-width:40ch;height:5.2em}',
    '.hero-story{position:relative;margin-top:26px;max-width:48ch;height:6.8em}'
)
html = html.replace(
    '.hero-story b{position:absolute;left:0;top:0;right:0;font-weight:400;\ncolor:var(--dim);will-change:opacity,transform;opacity:0}',
    '.hero-story b{position:absolute;left:0;top:0;right:0;font-weight:400;font-size:17.5px;line-height:1.6;\ncolor:rgba(242,244,247,.88);will-change:opacity,transform;opacity:0}'
)
html = html.replace(
    ...
    '.hero-story b i{display:block;font-family:var(--mono);font-size:11px;letter-spacing:.26em;\ncolor:var(--amb);margin-bottom:10px;font-style:normal}'
)

# Extra CSS for Chart.js canvases, live QEC status pill, and lattice
extra_css = """
/* ---- AdaptiveQEC Embedded Project Styles ---- */
.chart-box{width:100%;background:linear-gradient(160deg,#080b10,#050609);border:1px solid var(--hair);border-radius:10px;padding:24px;margin-top:24px;position:relative}
.chart-box canvas{width:100%!important;max-height:260px}
.hw-pill{display:inline-flex;align-items:center;gap:8px;padding:5px 14px;border-radius:99px;background:rgba(239,154,87,.12);border:1px solid rgba(239,154,87,.32);color:var(--amb);font-family:var(--mono);font-size:10px;letter-spacing:.2em}
.hw-pill .live-dot{width:6px;height:6px;border-radius:50%;background:var(--amb);box-shadow:0 0 8px var(--amb);animation:pulseDot 2s infinite}
@keyframes pulseDot{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.4;transform:scale(.8)}}
.lattice-wrap{width:100%;max-width:520px;aspect-ratio:520/380;border:1px solid var(--hair);border-radius:10px;background:linear-gradient(160deg,#080b10,#050609);padding:14px;position:relative}
.lattice-wrap svg{width:100%;height:100%;display:block}
.q-node{cursor:pointer;transition:transform .2s,fill .2s}
.q-node:hover{transform:scale(1.25);transform-origin:center}
.q-link{stroke:rgba(242,244,247,.18);stroke-width:1.5}
.q-link.active{stroke:var(--amb);stroke-width:2}
.inspect-banner{margin-top:16px;padding:16px 20px;border-radius:8px;background:rgba(14,21,31,.65);border:1px solid rgba(142,191,214,.25);display:flex;justify-content:space-between;align-items:center}
.inspect-banner b{font-size:16px;color:#fff}
.inspect-banner span{font-family:var(--mono);font-size:11px;color:var(--amb)}
"""
...
# Add live status indicator to top header
...
html = html.replace(
    ...
    '<div class="hero-spec mono"><span>IBM Eagle r3</span><span>27 Transmons</span><span>PL 0.0210</span><span>2.84 μs Decode</span></div>'
)

# =========================================================================
# 4. SECTION 02: TOPOLOGY & HEAVY-HEX LATTICE (Replaces FRACTURE copy)
# =========================================================================
html = html.replace(
...
# =========================================================================
# 5. SECTION 03: SYNDROME DEFECT LOUPE (Replaces LOUPE copy)
# =========================================================================
html = html.replace(
...
# =========================================================================
# 6. SECTION 04: DECODER BENCHMARKS (Replaces INDEX copy & adds Chart.js)
# =========================================================================
html = html.replace(
...
new_mat_rows = """<div class="rows" style="margin-top:0" id="matRows">
        ...
        <div class="chart-box" data-rv="" style="--i:8">
...
# =========================================================================
# 7. SECTION 05: IN-SITU DRIFT & CUSUM (Replaces DISPERSION copy)
# =========================================================================
html = html.replace(
...
new_lines = """<div class="lines">
      ...
      <!-- In-Situ Drift Chart -->
      <div class="chart-box" data-rv="" style="--i:7;margin-top:40px">
        <div style="display:flex;justify-content:space-between;margin-bottom:12px">
          <span class="mono">CUSUM DRIFT STATISTIC S(t) OVER 100 EXPERIMENT ROUNDS</span>
...
# =========================================================================
# 8. SECTION 06: DILUTION CRYOSTAT ASSEMBLY (Replaces ASSEMBLY copy)
# =========================================================================
html = html.replace(
...
# =========================================================================
# 9. SECTION 07: SELECTIVE RECALIBRATION (Replaces COATING copy)
# =========================================================================
html = html.replace(
...
# =========================================================================
# 10. SECTION 08: REALITY GAP & ERROR CLUSTERS (Replaces SPECIMENS copy)
# =========================================================================
html = html.replace(
...
# =========================================================================
# 11. SECTION 09: LIVE QEC WORKBENCH (Replaces WORKBENCH copy & wires live execution)
# =========================================================================
html = html.replace(
...
new_prop_rows = """<div class="rows" id="propRows">
      ...
      <div style="margin-top:24px;display:flex;gap:14px;align-items:center">
        <button class="btn p" id="btnRunLiveExperiment" style="cursor:pointer">EXECUTE REAL-TIME STIM EXPERIMENT</button>
        <button class="btn s" id="btnTriggerRecalibrate" style="cursor:pointer">TRIGGER SELECTIVE RECALIBRATION</button>
        <span class="mono" id="experimentStatusText" style="color:var(--faint);font-size:11px">READY // WAITING FOR TRIGGER</span>
      </div>"""
...
# =========================================================================
# 12. SECTION 10: QUANTUM STATE PROBABILITY FIELD (CAUSTICS)
# =========================================================================
html = html.replace(
...
# =========================================================================
# 15. SECTION 13: ANNEAL / FAULT TOLERANCE
# =========================================================================
html = html.replace(
...
# =========================================================================
# 17. LIVE CHART.JS INITIALIZATION & BACKEND API CONNECTORS
# =========================================================================
project_runtime_script = """
<script>
// Project Implementation: Live Charts, Heavy-Hex Lattice, and API Connectors
(function initAdaptiveQECProject() {
  console.log('[AdaptiveQEC] Project runtime initializing...');

  // 1. Initialize Decoder Benchmark Multi-Axis Chart
        ...
        datasets: [
          {
            ...
            backgroundColor: 'rgba(239, 154, 87, 0.75)',
            borderColor: '#ef9a57',
            borderWidth: 1.5,
            yAxisID: 'yLatency',
            borderRadius: 4
      ...
      options: {
        ...
        scales: {
          x: {
            ...
            ticks: { color: 'rgba(242,244,247,0.7)', font: { family: 'ui-monospace, monospace', size: 10 } }
          },
          yLatency: {
            type: 'linear',
            position: 'left',
            ...
            title: { display: true, text: 'Execution Latency (μs)', color: '#ef9a57' }
          },
          yError: {
            type: 'linear',
            position: 'right',
  ...
  // 2. Initialize In-Situ CUSUM Drift Control Chart
    ...
    new Chart(ctx, {
        ...
        datasets: [
          {
            ...
            data: cusumData,
            borderColor: '#8ebfd6',
            backgroundColor: 'rgba(142, 191, 214, 0.12)',
            fill: true,
            tension: 0.3,
            borderWidth: 2,
            pointRadius: 2
      ...
      options: {
        ...
        scales: {
          x: {
            grid: { color: 'rgba(242,244,247,0.06)' },
            ticks: { color: 'rgba(242,244,247,0.6)', maxTicksLimit: 10 }
          },
          y: {
            grid: { color: 'rgba(242,244,247,0.06)' },
            ticks: { color: 'rgba(242,244,247,0.75)' },
            title: { display: true, text: 'Cumulative Deviation S', color: 'rgba(242,244,247,0.7)' }
          }
        }
      }
    });
  ...
  // 3. Connect Live Stim Experiment Execution Button
  async function executeLiveExperiment() {
  ...
  // 4. Connect Selective Recalibration Trigger
  async function executeRecalibration() {
  ...
  if (runBtn) runBtn.addEventListener('click', executeLiveExperiment);
  ...
  if (heroRunBtn) heroRunBtn.addEventListener('click', (e) => {
    ...
    executeLiveExperiment();
  });

  const recalBtn = document.getElementById('btnTriggerRecalibrate');
  if (recalBtn) recalBtn.addEventListener('click', executeRecalibration);
...
html = html.replace('</body>', project_runtime_script + '\n</body>')

output_path = os.path.join('src', 'adaptive_qec', 'api', 'static', 'index.html')
with open(output_path, 'w', encoding='utf-8') as f:
    f.write(html)

print(f"Successfully generated complete AdaptiveQEC interface in {output_path} ({len(html)} bytes).")
